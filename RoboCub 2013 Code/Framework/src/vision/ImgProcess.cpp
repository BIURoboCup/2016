...
void ImgProcess::RGBtoHSV(FrameBuffer *buf) {
	...
	for (int i = 0; i < buf->m_RGBFrame->m_Width * buf->m_RGBFrame->m_Height;
			i++) {
		...
		buf->m_HSVFrame->m_ImageData[i * buf->m_HSVFrame->m_PixelSize + 0] =
				(unsigned char) (th >> 8);
		buf->m_HSVFrame->m_ImageData[i * buf->m_HSVFrame->m_PixelSize + 1] =
				(unsigned char) (th & 0xFF);
		buf->m_HSVFrame->m_ImageData[i * buf->m_HSVFrame->m_PixelSize + 2] =
				(unsigned char) (ts & 0xFF);
		buf->m_HSVFrame->m_ImageData[i * buf->m_HSVFrame->m_PixelSize + 3] =
				(unsigned char) (tv & 0xFF);
	}
}
...
void ImgProcess::Erosion(Image* img) {
	...
	for (y = 1; y < (img->m_Height - 1); y++) {
		for (x = 1; x < (img->m_Width - 1); x++) {
			temp_img[y * img->m_Width + x] = img->m_ImageData[(y - 1)
					* img->m_Width + (x - 1)]
...
void ImgProcess::Erosion(Image* src, Image* dest) {
	...
	for (y = 1; y < (src->m_Height - 1); y++) {
		for (x = 1; x < (src->m_Width - 1); x++) {
			dest->m_ImageData[y * src->m_Width + x] = src->m_ImageData[(y - 1)
					* src->m_Width + (x - 1)]
...
void ImgProcess::VFlipYUV(Image* img) {
	...
	for (int h = 0; h < img->m_Height / 2; h++) { /*line iterator*/
		...
		memcpy(line2, img->m_ImageData + (img->m_Height - 1 - h) * sizeline,
				sizeline);

		memcpy(img->m_ImageData + h * sizeline, line2, sizeline);
		memcpy(img->m_ImageData + (img->m_Height - 1 - h) * sizeline, line1,
				sizeline);
	}
}
