...
void LinuxCamera::ErrorExit(const char* s)
{
    ...
    printf("try to do this:\n sudo killall RoBIU -9\n");
    printf("or try to do this:\n");
	printf("sudo modprobe -r uvcvideo\n");
	printf("sudo modprobe -r uvcvideo\n");
...
int LinuxCamera::Initialize(int deviceIndex)
{
    ...
    fps.parm.capture.timeperframe.denominator = 14;
...
int LinuxCamera::MyReadFrame()
{
    ...
    if (-1 == ioctl (camera_fd, VIDIOC_DQBUF, &buf)) {
        ...
        case EAGAIN:
#ifdef CAMERA_DEBUG
        printf("IOCTL VIDIOC_DQBUF EAGAIN\n");
#endif
        ...
        default:
#ifdef CAMERA_DEBUG
        printf("IOCTL VIDIOC_DQBUF OTHER ERROR, Exiting...\n");
#endif
    ...
    //ImgProcess::VFlipYUV(fbuffer->m_YUVFrame);
#ifdef CAMERA_DEBUG
        printf("Before conversion from YUV to RGB\n");
#endif
    ImgProcess::YUVtoRGB(fbuffer);
 //   ImgProcess::RGBtoHSV(fbuffer);
#ifdef CAMERA_DEBUG
        printf("After conversion from YUV to RGB\n");
#endif
    if (-1 == ioctl (camera_fd, VIDIOC_QBUF, &buf))
    {
#ifdef CAMERA_DEBUG
        printf("IOCTL VIDIOC_QBUF Error, Exiting\n");
#endif
...
#ifdef CAMERA_DEBUG
		printf("NOT EINTR error, Exiting...\n");
#endif
