...
void httpd::command(int fd, char *parameter) {
  ...
  if ( (svalue = strstr(parameter, "value=")) != NULL ) {
    ...
    ivalue = MAX(MIN(strtol(svalue, NULL, 10), 76800), -999);
    fvalue = MAX(MIN(strtof(svalue, NULL), 76800), -999);
...
*/
  ...
  // if (strncmp("calibrate_pixel", command, strlen("calibrate_pixel") ) == 0)
   //{
   //   input_cmd(IN_CMD_CALIBRATE_PIXEL, fvalue, ret_s);
  // }
