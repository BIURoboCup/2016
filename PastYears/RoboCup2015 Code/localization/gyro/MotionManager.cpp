...
void MotionManager::Process()
{
    ...
    if(m_CalibrationStatus == 1 && m_Enabled == true)
    {
        ...
        int sum_fb = 0, sum_rl = 0, avr_fb, avg_rl;
        for(int idx = 0; idx < ACCEL_WINDOW_SIZE; idx++)
        {
            sum_fb += fb_array[idx];
            sum_rl += rl_array[idx];
        }
        avr_fb = sum_fb / ACCEL_WINDOW_SIZE;
        avg_rl = sum_rl / ACCEL_WINDOW_SIZE;

        //std::cout << "avr_fb: " << avr_fb << std::endl;////////////////////
        //std::cout << "avg_rl: " << avg_rl << std::endl;////////////////////

//        if((avr_fb < MotionStatus::FALLEN_F_LIMIT) || (avr_fb > MotionStatus::FALLEN_B_LIMIT))	{ //check fallen front back
//
//        	BiuPlayer::getInstance()->m_biumotion->turnOffAllMX28s();
//        	sleep(5);
//        	exit(0);////////////////////////////////////
//        }
        ...
        if(avr_fb < MotionStatus::FALLEN_F_LIMIT)	//check fallen front back
            MotionStatus::FALLEN = FORWARD;
        else if(avr_fb > MotionStatus::FALLEN_B_LIMIT)
            MotionStatus::FALLEN = BACKWARD;
        else if(avg_rl < MotionStatus::FALLEN_R_LIMIT)	//check fallen right left
            MotionStatus::FALLEN = RIGHT;
        else if(avg_rl > MotionStatus::FALLEN_L_LIMIT)
