...
static int getkeystring(INI_FILETYPE *fp, const TCHAR *Section,
		const TCHAR *Key, int idxSection, int idxKey, TCHAR *Buffer,
		int BufferSize) {
	TCHAR *sp, *ep;
	...
	do {
		...
		if (ep == NULL)
			...
			|| (((int) (skiptrailing(ep, sp) - sp) != len
					|| _tcsnicmp(sp, Key, len) != 0) && ++idx != idxKey));
	if (idxKey >= 0) {
		if (idx == idxKey) {
			assert(ep != NULL);
			assert(*ep == '=' || *ep == ':');
...
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value,
		...
		do {
			if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
				...
				if (Key != NULL && Value != NULL) {
					...
					/* force a new line (there may not have been one) behind the last line of the INI file */
					writesection(LocalBuffer, Section, &wfp);
					writekey(LocalBuffer, Key, Value, &wfp);
				} /* if */
				/* Clean up and rename */
	...
	for (;;) {
		...
		if (ep == NULL)
			ep = _tcschr(sp, ':');
		match = (ep != NULL && (int) (skiptrailing(ep, sp) - sp) == len
	...
	if (*sp == '[') {
		...
		if (Key != NULL && Value != NULL) {
			...
			ini_write(INI_LINETERM INI_LINETERM, &wfp);
			/* put a blank line between the current and the next section */
		} /* if */
