...
void Action::Process() {
    ...
    * -----/  |        |  |    |   \----
	...
	if (m_FirstDrivingStart == true) // ó�� �����Ҷ�
		...
		for (bID = JointData::ID_R_SHOULDER_PITCH;
				bID < JointData::NUMBER_OF_JOINTS; bID++) {
			if (m_Joint.GetEnable(bID) == true) {
				wpTargetAngle1024[bID] = MotionStatus::m_CurrentJoints.GetValue(
						bID);
	...
	if (wUnitTimeCount < wUnitTimeNum) // ���� �������̶��
				...
				if (m_Joint.GetEnable(bID) == true) {
					if (ipMovingAngle1024[bID] == 0)
						...
						if (bSection == PRE_SECTION) {
							iSpeedN = (short) (((long) (ipMainSpeed1024[bID]
									- ipLastOutSpeed1024[bID]) * wUnitTimeCount)
									/ wUnitTimeNum);
							ipGoalSpeed1024[bID] = ipLastOutSpeed1024[bID]
									+ iSpeedN;
							ipAccelAngle1024[bID] =
									(short) ((((long) (ipLastOutSpeed1024[bID]
											+ (iSpeedN >> 1)) * wUnitTimeCount
											* 144) / 15) >> 9);
							...
							m_Joint.SetValue(
									bID,
									wpStartAngle1024[bID]
											+ (short int) (((long) (ipMainAngle1024[bID])
													* wUnitTimeCount)
													/ wUnitTimeNum));
						...
						} else // POST_SECTION
						{
							if (wUnitTimeCount == (wUnitTimeNum - 1)) {
								...
								if (bpFinishType[bID] == ZERO_FINISH) {
									iSpeedN = (short int) (((long) (0
											- ipLastOutSpeed1024[bID])
											* wUnitTimeCount) / wUnitTimeNum);
									ipGoalSpeed1024[bID] =
											ipLastOutSpeed1024[bID] + iSpeedN;
									m_Joint.SetValue(
											bID,
											wpStartAngle1024[bID]
													+ (short) ((((long) (ipLastOutSpeed1024[bID]
					...
					m_Joint.SetSlope(bID,
							1 << (m_PlayPage.header.slope[bID] >> 4),
							1 << (m_PlayPage.header.slope[bID] & 0x0f));
					m_Joint.SetPGain(bID,
							(256 >> (m_PlayPage.header.slope[bID] >> 4)) << 2);
				}
			}
		}
		...
		if (bSection == PRE_SECTION) {
				...
				if (m_Joint.GetEnable(bID) == true) {
					if (bpFinishType[bID] == NONE_ZERO_FINISH) {
						if ((wUnitTimeTotalNum - wAccelStep) == 0) // ��� ������ ���� ��ٸ�
							...
							ipMainAngle1024[bID] =
									(short) ((((long) (ipMovingAngle1024[bID]
											- ipAccelAngle1024[bID]))
											* wUnitTimeNum)
											/ (wUnitTimeTotalNum - wAccelStep));
					} else
						// ZERO_FINISH
						ipMainAngle1024[bID] = ipMovingAngle1024[bID]
								- ipAccelAngle1024[bID]
								- (short int) ((((long) ipMainSpeed1024[bID]
										* wAccelStep * 12) / 5) >> 8);
				}
			}
		} else if (bSection == MAIN_SECTION) {
			...
			for (bID = JointData::ID_R_SHOULDER_PITCH;
				...
				if (m_Joint.GetEnable(bID) == true)
					ipMainAngle1024[bID] = ipMovingAngle1024[bID]
							- ipMainAngle1024[bID] - ipAccelAngle1024[bID];
			}
		...
		if (bSection == PRE_SECTION) {
			...
			if (m_PageStepCount == m_PlayPage.header.stepnum) // ������ �����̶��
				...
				if (wNextPlayPage == 0) // ����� ���� �������� ��ٸ� ���� ���ܱ����ϰ� ����
					...
					// ����� ������ ��ٸ� ���� ���ܱ����ϰ� ����
					if (m_NextPlayPage.header.repeat == 0
							|| m_NextPlayPage.header.stepnum == 0)
						m_PlayingFinished = true;
				}
			}
			...
			//////// Step �Ķ���� ���
			wPauseTime =
					(((unsigned short) m_PlayPage.step[m_PageStepCount - 1].pause)
							<< 5) / m_PlayPage.header.speed;
			wMaxSpeed256 =
					((unsigned short) m_PlayPage.step[m_PageStepCount - 1].time
			...
			for (bID = JointData::ID_R_SHOULDER_PITCH;
				...
				if (m_Joint.GetEnable(bID) == true) {
					...
					if (m_PlayPage.step[m_PageStepCount - 1].position[bID]
							& INVALID_BIT_MASK)
						wCurrentTargetAngle = wpTargetAngle1024[bID];
					else
						wCurrentTargetAngle = m_PlayPage.step[m_PageStepCount
								- 1].position[bID];
					...
					// Find Moving offset
					ipMovingAngle1024[bID] = (int) (wpTargetAngle1024[bID]
							- wpStartAngle1024[bID]);
					...
					if (m_PageStepCount == m_PlayPage.header.stepnum) // ���� ������ �������̶��
						...
						if (m_PlayingFinished == true) // ���� �����̶��
							...
							if (m_NextPlayPage.step[0].position[bID]
									& INVALID_BIT_MASK)
								wNextTargetAngle = wCurrentTargetAngle;
							else
								wNextTargetAngle =
										m_NextPlayPage.step[0].position[bID];
						}
					} else {
						if (m_PlayPage.step[m_PageStepCount].position[bID]
								& INVALID_BIT_MASK)
							wNextTargetAngle = wCurrentTargetAngle;
						else
							wNextTargetAngle =
									m_PlayPage.step[m_PageStepCount].position[bID];
					}

					// Find direction change
					if (((wPrevTargetAngle < wCurrentTargetAngle)
							&& (wCurrentTargetAngle < wNextTargetAngle))
							|| ((wPrevTargetAngle > wCurrentTargetAngle)
									&& (wCurrentTargetAngle > wNextTargetAngle))) {
						// ��� �����ϰų� �����ϰ�, Ȥ�� ���ٸ�(��, �ҿ��� ���� ��ٸ�)
						bDirectionChanged = 0;
					} else {
					...
					// Find finish type
					if (bDirectionChanged || wPauseTime
							|| m_PlayingFinished == true) {
			...
			for (bID = JointData::ID_R_SHOULDER_PITCH;
				...
				if (m_Joint.GetEnable(bID) == true) {
					lStartSpeed1024_PreTime_256T =
							(long) ipLastOutSpeed1024[bID]
									* ulPreSectionTime256T; //  *300/1024 * 1024/720 * 256 * 2
					lMovingAngle_Speed1024Scale_256T_2T =
							(((long) ipMovingAngle1024[bID]) * 2560L) / 12;

					if (bpFinishType[bID] == ZERO_FINISH)
						ipMainSpeed1024[bID] =
								(short int) ((lMovingAngle_Speed1024Scale_256T_2T
										- lStartSpeed1024_PreTime_256T)
										/ lDivider2);
					else
						ipMainSpeed1024[bID] =
