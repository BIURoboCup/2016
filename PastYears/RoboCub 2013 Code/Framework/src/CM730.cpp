...
//#define DEBUG_VISION_CM

BulkReadData::BulkReadData() :
		start_address(0), length(0), error(-1) {
...
int CM730::TxRxPacket(unsigned char *txpacket, unsigned char *rxpacket,
		int priority) {
	if (priority > 1)
	...
	if (length < (MAXNUM_TXPARAM + 6)) {
		...
		if (m_Platform->WritePort(txpacket, length) == length) {
			if (txpacket[ID] != ID_BROADCAST) {
				...
				while (1) {
					length = m_Platform->ReadPort(&rxpacket[get_length],
					...
					if (get_length == to_length) {
						...
						for (i = 0; i < (get_length - 1); i++) {
							if (rxpacket[i] == 0xFF && rxpacket[i + 1] == 0xFF)
								break;
							else if (i == (get_length - 2)
									&& rxpacket[get_length - 1] == 0xFF)
								break;
						}
				...
				while (1) {
					...
					if (i == 0) {
						...
						if (rxpacket[LENGTH + rxpacket[LENGTH]] == checksum) {
							...
							m_BulkReadData[rxpacket[ID]].error =
									(int) rxpacket[ERRBIT];

							int cur_packet_length = LENGTH + 1
									+ rxpacket[LENGTH];
...
void CM730::Disconnect() {
	...
	unsigned char txpacket[] = { 0xFF, 0xFF, 0xC8, 0x05, 0x03, 0x1A, 0xE0, 0x03,
			0x32 };
...
int CM730::ReadTable(int id, int start_addr, int end_addr, unsigned char *table,
		int *error) {
	unsigned char txpacket[MAXNUM_TXPARAM + 10] = { 0, };
	unsigned char rxpacket[MAXNUM_RXPARAM + 10] = { 0, };
	int result;
...
int CM730::WriteWord(int id, int address, int value, int *error) {
	...
	txpacket[LENGTH] = 5;
#ifdef DEBUG_VISION_CM
	if (address == CM730::P_LED_HEAD_L && value == CM730::MakeColor(255, 0, 0))
	{
		printf("Vision:: in WriteWord to head\n");
	}
#endif
	result = TxRxPacket(txpacket, rxpacket, 2);
#ifdef DEBUG_VISION_CM
	if (address == CM730::P_LED_HEAD_L && value == CM730::MakeColor(255, 0, 0))
	{
		printf("Vision:: in WriteWord to head, after TxRxPacket. result = %d\n", result);
	}
#endif
	if (result == SUCCESS && id != ID_BROADCAST) {
		if (error != 0)
